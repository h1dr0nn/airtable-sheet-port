...
impl SheetPortServer {
    ...
    #[tool(
        name = "append_records",
        description = "Stage new rows to append to a table and return the pending change. Works on an empty tab too: if the sheet has no header yet, the field names of the records you pass become the header row (row 1) and the records are written below it, so you can populate a blank sheet in one call - never refuse to write just because a sheet is empty. This does NOT write anything: it returns a changeId you then pass to commit_change. Some changes require the user to approve them in the desktop app before commit_change will apply them. For Google Sheets the tableId may be a Google Sheets URL, a bare spreadsheet id, or spreadsheetId:gid / spreadsheetId:SheetName to pick a specific tab."
    )]
